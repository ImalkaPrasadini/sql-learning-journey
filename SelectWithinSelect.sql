/*
SELECT within SELECT Tutorial SQL Zoo
*/
...
CONCAT(cast(ROUND
(population/
(select population from world where name = 'Germany'),2)*100 as int),'%') as percentage
from world
...
select name from world
where gdp > (select max(ifnull(gdp,0)) from world where continent = 'Europe')
